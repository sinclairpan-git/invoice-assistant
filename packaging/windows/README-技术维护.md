...
- 交付形态为单目录便携包
- 首选本地访问地址：`http://127.0.0.1:18080`
- 若 `18080` 被其他程序占用，便携包会按 `18081-18089` 的固定端口池顺序回退
...
- `--port 18080` 表示首选端口，不表示“只能监听 18080”；若被占用，bootstrap 会在 `18080-18089` 内选择第一个可用端口
...
- `logs/startup.log`：启动器和 `start_server.py` 的最早期启动轨迹；即使 Python 进程在导入阶段失败，也应在这里留下痕迹
...
1. 优先手动访问 `http://127.0.0.1:18080`
2. 如果 `18080` 不通，检查 `runtime/app.url`，按其中记录的实际地址访问
2. 如果无法访问，检查 `logs/`
...
- 服务只允许监听 `127.0.0.1` loopback，且只会在固定端口池 `18080-18089` 内选择端口
